...
struct SplayTreeNode
{
    ...
    SplayTreeNode *left;
    SplayTreeNode *right;
    SplayTreeNode(string k) : key(k), left(nullptr), right(nullptr) {}
};
...
private:
    ...
    SplayTreeNode *splay(SplayTreeNode *root, const string &key);
...
// Perform splay operation
SplayTreeNode *SplayTree::splay(SplayTreeNode *root, const string &key)
{
...
// Left rotate
SplayTreeNode *SplayTree::leftRotate(SplayTreeNode *x)
{
...
// Right rotate
SplayTreeNode *SplayTree::rightRotate(SplayTreeNode *x)
{
...
{
    // List of songs
    vector<string> songs = {"1.mpeg", "2.mpeg", "3.mpeg", "4.mpeg", "5.mpeg", "6.mpeg", "7.mpeg", "8.mpeg", "9.mpeg"};
    vector<string> song_name = {"ANBAE PERANBAE", "MARUVARTHAI", "THULI THULI", "VIZHI MOODI", "VENMATHIYE", "KURUMUGIL", "PO INDRU NEEYAGA", "POONGATRILE", "MUZHUMADHI"};
    // List of corresponding images
    vector<string> images = {"1.jpeg", "2.jpeg", "3.jpeg", "4.jpeg", "5.jpeg", "6.jpeg", "7.jpeg", "8.jpeg", "9.jpeg"};
...
<body>
    ...
    for (const string &song : songs)
    {
        int index = find(songs.begin(), songs.end(), song) - songs.begin();
        htmlContent += "                <li onclick=\"playSong('" + song + "')\">" + song_name[index] + "</li>\n";
    }
    ...
    // Add song options in the dropdown
    for (const string &song : songs)
    {
        int index = find(songs.begin(), songs.end(), song) - songs.begin();
        htmlContent += "                <option value=\"" + song + "\">" + song_name[index] + "</option>\n";
    }
    ...
    ofstream outFile("ashify.html");
    outFile << htmlContent;
    outFile.close();

    cout << "HTML file 'ashify.html' has been created." << endl;
